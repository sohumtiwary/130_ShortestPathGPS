...
void drawMap(sf::RenderWindow &window, const vector<City> &cities, const vector<int> &route) {
    ...
    for (const auto& city : cities) {
        ...
        sf::Font font;
        if (!font.loadFromFile("arial.ttf")) {  // Make sure you have a font file in the same directory
    ...
    for (size_t i = 0; i < route.size() - 1; ++i) {
        sf::Vertex line[] = {
            sf::Vertex(sf::Vector2f(cities[route[i]].x + 10, cities[route[i]].y + 10), sf::Color::Blue),
            sf::Vertex(sf::Vector2f(cities[route[i + 1]].x + 10, cities[route[i + 1]].y + 10), sf::Color::Blue)
        };
