...
public:
    ...
    void parseFile(const string &filename)
    {
        ...
        // --- read city names from the next non-empty line ---
        string line;
        do
        {
            if (!getline(in, line))
            {
                cerr << "Unexpected EOF reading city names\n";
                exit(1);
            }
...

        {
            ...
            if ((int)cityNames.size() != numberOfNodes)
            {
                cerr << "Expected " << numberOfNodes << " city names but got "
                     << cityNames.size() << "\n";
            }
        }

        // initialize adjacencyMatrix
        adjacencyMatrix.assign(numberOfNodes,
                               vector<int>(numberOfNodes, INF));
        for (int i = 0; i < numberOfNodes; i++)
            adjacencyMatrix[i][i] = 0;

        // --- read exactly M edges, skipping comments/blank lines ---
        int edgesRead = 0;
        while (edgesRead < M && getline(in, line))
        {
            ...
            if (!(ess >> c1 >> c2 >> dist))
            {
                cerr << "Warning: malformed edge line, skipping: " << line << "\n";
                continue;
            }
        ...
        if (edgesRead < M)
        {
            cerr << "Warning: only read " << edgesRead
                 << " edges, expected " << M << "\n";
        }
    }
